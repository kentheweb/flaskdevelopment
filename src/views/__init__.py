...
def home():
    if 'username' in session:
        ...
        return render_template('posts/home.html', post=post, user=user, count=count, d=d)
    else:
...
def account():
    ...
    if 'username' in session:
        ...
        if request.method == 'GET':
            return render_template('account.html', member=member, user=user, count=count, quiz=quiz, answer=answer)
        else:
